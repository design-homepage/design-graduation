...
/* ✅ 진짜 중앙 sticky (그대로 유지) */
const StickyFrame: React.FC<{ children: React.ReactNode; autoCenter?: boolean }> = ({
    children,
    autoCenter = true,
}) => {
            ...
            <div className="flex items-start gap-12 px-10 pr-14">
                {items.map(({ src, label }, i) => (
                    <figure
                        ...
                        style={{ height: "689px" }}
                    >
                        <figcaption className="mb-4 text-center text-[20px] font-medium">
                            {label}
                        </figcaption>
                        <img
                            src={src}
                            alt={label}
                            className="max-h-[689px]"
                            draggable={false}
...
const AboutInfoSection: React.FC = () => {
    return (
        <section
            ...
            style={{ width: PAGE_W, height: PAGE_H, padding: `${PAD_Y}px ${PAD_X}px` }}
        >
